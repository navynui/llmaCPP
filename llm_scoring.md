...
| 5 | `Qwen3.6-35B-A3B-Uncensored-HauhauCS-Aggressive-IQ3_M` | 31.8 | **13/25** | **13/18** | **16/18** | **5/15** | **13/14** | **9/10** | **🏅 69/100** |
| 5 | `Qwen3.6-35B-A3B-UD-IQ3_S` | 32.8 | **14/25** | **15/18** | **16/18** | **5/15** | **11/14** | **8/10** | **🏅 69/100** |
| 5 | `Qwopus3.6-35B-A3B-v1-IQ4_XS` | 33.2 | **14/25** | **16/18** | **17/18** | **8/15** | **5/14** | **9/10** | **🏅 69/100** |
| 6 | `gemma-4-E4B-it-Q4_K_M` | 35.3 | **15/25** | **14/18** | **13/18** | **10/15** | **9/14** | **6/10** | **🏅 67/100** |
| 6 | `Qwen3.5-9B-Claude-HighIQ-THINKING-HERETIC-UNCENSORED-4.6-Q4_K` | 29.6 | **12/25** | **12/18** | **14/18** | **7/15** | **13/14** | **9/10** | **🏅 67/100** |
| 6 | `granite-4.1-8b-Q4_K_M` | ~29.5 | **12/25** | **17/18** | **17/18** | **6/15** | **9/14** | **6/10** | **🏅 67/100** |
| 11 | `Hermes-3-Llama-3.2-3B.Q5_K_M` | 60.6 | **25/25** | **3/18** | **9/18** | **4/15** | **13/14** | **7/10** | **🔹 63/100** |
| 12 | `Qwen3.6-14B-A3B-VibeForged-v2-Q4_K_M` | 39.8 | **17/25** | **11/18** | **14/18** | **3/15** | **13/14** | **3/10** | **🔹 61/100** |
| 13 | `oh-dcft-v3.1-gpt-4o-mini.Q4_K_M` | 34.4 | **14/25** | **8/18** | **10/18** | **5/15** | **7/14** | **6/10** | **🔹 50/100** |
| 15 | `Qwen3.5-9B-Uncensored-HauhauCS-Aggressive-Q4_K_M` | 29.4 | **12/25** | **8/18** | **7/18** | **6/15** | **5/14** | **4/10** | **🔹 42/100** |
...
| 5 | `Qwen3.6-35B-A3B-Uncensored-HauhauCS-Aggressive-IQ3_M` | **13** (31.8 t/s) | Good TokenBucket class & connection pooling but code is incomplete/cut off mid-implement; rate limiter logic is sound | Excellent technical reasoning on KV cache allocation; detailed block-chunk model analysis with consumer GPU trade-offs | Hallucinates Thai spelling in full name; verbose CoT with multiple self-corrections | Strong mathematical derivation with step-by-step matrix transformations and algebraic verification | Excellent cyberpunk hard-boiled narrative; all 3 technical details accurate; tight first-person isolation atmosphere |
...
| 5 | `Qwopus3.6-35B-A3B-v1-IQ4_XS` | **14** (33.2 t/s) | Near-complete, well-architected asyncio scraper with BeautifulSoup; hit 4096-token cap just before final output block | Excellent structured explanation of continuous batching with accurate comparison table for static vs paged memory | Deep reasoning shown in `<think>` chain but hit 4096-token limit before delivering a clean formatted answer | Hit 4096-token context limit mid-derivation; no final answer delivered despite correct intermediate math | Excellent hard-boiled cyberpunk story; precise BGP/LSP/OSPF details; strong isolation atmosphere; clean under 500-word delivery |
...
| 6 | `Qwen3.5-9B-Claude-HighIQ-THINKING-HERETIC-UNCENSORED-4.6-Q4_K` | **12** (29.6 t/s) | Logical structure and token bucket correct; sequential `gather` in loop negates concurrency; missing `aiohttp` import; misaligned `except` block | Solid chunked-buffer + RoPE scaling explanation with practical recommendations; minor inaccuracies on specific internal mechanisms | Incomplete: provided short-form name only (`Krung Thep Maha Nakhon`); skipped full Thai script and attribution is incorrect (Rama IV / 1860) | Correct 1-based formula `(N-j+1, N-i+1)` with clean step-by-step derivation | Standout cyberpunk noir story; all 3 technical details included (latency spike, packet loss, silent failover); tight first-person voice; strong atmosphere |
| 6 | `granite-4.1-8b-Q4_K_M` | **12** (29.5 t/s) | Highly optimized Python scripts with custom rate limiters & backoff | Exceptionally structured, clear explanations of paged attention vs static buffers | Hallucinates formal names & factual QA; requires strict constraints for accuracy | Moderate abstract reasoning capability | Straightforward, highly predictable text generation |
| 11 | `Hermes-3-Llama-3.2-3B` | **25** (60.6 t/s) | Skeleton code with placeholder comments requiring human intervention | Plausible but slightly hallucinated memory management explanations | Incorrect factual translations & formal city names | Excellent mathematical formula derivation `(N-j, i)` | Surprisingly competent narrative generation for a sub-4GB model |
| 12 | `Qwen3.6-14B-A3B-VibeForged-v2-Q4_K_M` | **17** (39.8 t/s) | Significant bugs: missing `await` on `response.text()`, broken rate limiter, `limiter.release()` does not exist, results from `gather` are raw responses | Verbose and detailed; plausible overall but fabricates specifics ("lock-free allocation queue", "periodic rebalancer every 10s", non-existent `nvidia-smc`) | Severe hallucination: invented "Bangkok City" with fabricated etymology; no Thai script provided | Correct final answer `(N-1-j, N-1-i)` with solid 0-indexed step-by-step proof and algebraic verification | Severe repetition loop after first ~500 words; narrative quality decays rapidly to copy-pasted sentences until hitting 4096-token limit |
| 13 | `oh-dcft-v3.1-gpt-4o-mini` | **14** (34.4 t/s) | Buggy code snippets requiring significant rewriting | Shallow technical explanations that miss architectural nuances | Severe factual hallucination under open-ended QA prompts | Superficial abstract logic handling | Decent creative writing but struggles with long-form coherence |
| 15 | `Qwen3.5-9B-Uncensored` | **12** (29.4 t/s) | Fast but simplified code output lacking error handling | Superficial technical explanations missing key implementation details | Limited factual knowledge base due to smaller parameter count | Weak multi-step logic; prone to early termination errors | Basic narrative generation with limited plot depth |
